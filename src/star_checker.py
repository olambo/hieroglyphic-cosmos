...
def query_star_coordinates(star_name):
    ...
    try:
        ...
        plx = result['PLX_VALUE'][0] if 'PLX_VALUE' in result.colnames and result['PLX_VALUE'][0] else None
        distance = None
        if plx and plx > 0:
            distance = round(1000.0 / plx, 2)  # Convert parallax (mas) to distance (ly approximation)
...
def print_summary(results, errors):
    ...
    print(f"✓ {len([r for r in results if not r['significant_change']])} "
